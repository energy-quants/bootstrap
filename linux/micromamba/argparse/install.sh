...
# ARG_OPTIONAL_BOOLEAN([cleanup],[],[Cleans caches/temp files after install.],[on])
# ARG_DEFAULTS_POS([])
...
print_help()
{
	...
	printf 'Usage: %s [-v|--version <arg>] [--(no-)cleanup] [-h|--help]\n' "$0"
	printf '\t%s\n' "-v, --version: The version of mambaforge to download and install. (default: 'latest')"
	printf '\t%s\n' "--cleanup, --no-cleanup: Cleans caches/temp files after install. (on by default)"
	printf '\t%s\n' "-h, --help: Prints help"
}
